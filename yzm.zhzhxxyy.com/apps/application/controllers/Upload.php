...
class Upload extends CI_Controller {
	...
	public function index(){
		...
		$data['uploadsave'] = site_url('upload/save/'.$cid);
	...
	public function save($cid=0){
		...
		if(substr($Video_Path,strlen($Video_Path)-1,1)=="/"){
            $Video_Path=substr($Video_Path,0,strlen($Video_Path)-1);
        }
		...
		if(!isset($_FILES['video']['name'])){
			$file_name = rand(111111,999999).time().'.mp4';
		}
		//文件后缀
	    ...
	    $path="/www/wwwroot/yzm.zhzhxxyy.com/video/data/2019/09/21/2019092111430115135.mp4";
        $this->load->library('xyz');
        $format = $this->xyz->format($path);
